      ...
      <table className='text-sm text-left sm:w-7/12 w-full max-w mx-auto'>
        <thead className='text-xs text-gray-700 uppercase border-b '>
          <tr>
            ...
            <th scope='col' className='px-6 py-3 w-0 font-medium'>Explanation</th>
          </tr>
        </thead>
        <tbody>
          {problemsList.map((problem: Problem, idx: number) => {
            ...
            return (
              <tr className={`${idx % 2 == 1 ? "bg-zinc-100" : ""}`} key={problem.id}>
                ...
                <td className={'px-6 py-4'}>
                  <Link className='hover:text-blue-600 cursor-pointer' href={`/problems/${problem.slug}/explanation`}>
                    View
                  </Link>
                </td>
              </tr>
